...
liftPreOpenAcc liftA pacc =
  let
      ...
      -- liftB :: Boundary (EltRepr e) -> Q (TExp (Boundary (EltRepr e)))
...
liftPreOpenExp liftA pexp =
  ...
  case pexp of
    ...
    IndexAny                  -> [|| IndexAny ||]
    -- IndexSlice slice prox sh  -> [|| IndexSlice $$(liftSliceIndex slice) $$(prox) $$(liftE sh) ||]
...
liftArray :: forall sh e. Shape sh => Array sh e -> Q (TExp (Array sh e))
liftArray (Array sh adata) =
  ...
  where
    sz :: Int
    sz | Just Refl <- (eqT :: Maybe (EltRepr sh :~: sh)) = size sh
       | otherwise = error "Error in liftArray"
